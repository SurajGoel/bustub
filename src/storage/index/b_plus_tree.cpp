...
auto BPLUSTREE_TYPE::GetValue(const KeyType &key, std::vector<ValueType> *result, Transaction *transaction) -> bool {
  auto curr_raw_page = buffer_pool_manager_->FetchPage(root_page_id_);
  curr_raw_page->RLatch();

  auto curr_page = reinterpret_cast<BPlusTreePage *>(curr_raw_page->GetData());
  //  auto curr_page = reinterpret_cast<BPlusTreePage *>(buffer_pool_manager_->FetchPage(root_page_id_)->GetData());
  // Get a read latch and then get a read latch
  ...
  while (!curr_page->IsLeafPage()) {
    ...
    // First take read latch on the next page
    auto next_page_raw = buffer_pool_manager_->FetchPage(next_page_id);
    next_page_raw->RLatch();
    curr_raw_page->RUnlatch();

    curr_raw_page = next_page_raw;
    curr_page = reinterpret_cast<BPlusTreePage *>(next_page_raw->GetData());
  }
...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value, Transaction *transaction) -> bool {
  ...
  for (int idx = 0; idx < curr_page_as_leaf->GetSize(); idx++) {
    auto curr_key = curr_page_as_leaf->KeyAt(idx);
    if (comparator_(curr_key, key) == 0) {  // Key is already present
      return false;
    }
  }
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::Begin() -> INDEXITERATOR_TYPE { return INDEXITERATOR_TYPE(buffer_pool_manager_); }
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::End() -> INDEXITERATOR_TYPE { return INDEXITERATOR_TYPE(buffer_pool_manager_); }
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::InsertIntoInternalPage(InternalPage *internalPage, const KeyType &key, const page_id_t &value)
    -> bool {
  if (internalPage->GetSize() == internal_max_size_) {
    ...
    for (int i = mid; i < internalPage->GetSize(); i++) {
      page_id_t child_page_id = internalPage->ValueAt(i);
      new_internal_page->PutKeyValuePairAt(i - mid, std::make_pair(internalPage->KeyAt(i), child_page_id));
    ...
    (index_to_insert < internal_max_size_ && index_to_insert <= mid)
    ...
    if (parent_page_id == INVALID_PAGE_ID) {
      ...
      auto new_parent_page =
          reinterpret_cast<InternalPage *>(buffer_pool_manager_->NewPage(&new_parent_page_id)->GetData());
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::SplitLeafPageAndAddKey(BPlusTree::LeafPage *leafPage, const KeyType &key, const ValueType &value)
    -> page_id_t {
  ...
  (index_to_insert < leaf_max_size_ && index_to_insert <= mid)
      ? leafPage->InsertKVPairAt(index_to_insert, std::make_pair(key, value))
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::SplitInternalPageAndAddKey(BPlusTree::LeafPage *leafPage, const KeyType &key,
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::InsertAndUpdateParentPage(BPlusTree::InternalPage *insertTo, int idx, const KeyType &key,
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::MergeLeafNode(BPlusTree::LeafPage *fromLeafPage, BPlusTree::LeafPage *toLeafPage) -> bool {
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::MergeInternalNode(BPlusTree::InternalPage *fromPage, BPlusTree::InternalPage *toPage) -> bool {
